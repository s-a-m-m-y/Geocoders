    ...
    with open( input_csv_file ) as csv_file:
        with open( output_csv_file, 'wb' ) as out_csv_file:
            ...
            for row in reader:
                ...
                row['google url'] = lat_lng['url']
                row['_geocode_status'] = lat_lng['geocode_status']
                if lat_lng['result'] is not None:
                    try:
                      row['_lat'] = lat_lng['result']['location']['lat']
                      row['_lng'] = lat_lng['result']['location']['lng']
                      row['_location_type'] = ''.join(lat_lng['result']['location_type'])
                      if writer is None:
                          keys = row.keys()
                          keys.sort()
                          writer = csv.DictWriter( out_csv_file, fieldnames=list(keys) )
...
def parse_api_results ( data ):
    for result_row in data['results'] :
        if ( (result_row['geometry']['location_type'] == "ROOFTOP") or
           (result_row['geometry']['location_type'] == "RANGE_INTERPOLATED") ):
